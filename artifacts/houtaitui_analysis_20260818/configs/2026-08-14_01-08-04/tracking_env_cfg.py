...
# Action EMA used by the houtaituiEMA task only.  The original alpha=0.5 run
# was visibly behind the unfiltered baseline during early single-support
# learning; alpha=0.7 keeps smoothing while reducing task-band phase lag.
...
class EmaDeploymentSafeActionsCfg(DeploymentSafeActionsCfg):
    ...
    them.  At alpha=0.7 the 12 Hz loop gain is about 0.68 while the task band
    below 3 Hz loses less than 0.4 dB.
