...
class CommandsCfg:
    ...
    motion = mdp.MotionCommandCfg(
        asset_name="robot",
        motion_file=f"{os.path.dirname(__file__)}/../jump/jump.npz",
...
class EventCfg(BaseEventCfg):
    """Reduced randomization for the first jump training stage."""
...
class TerminationsCfg(BaseTerminationsCfg):
    """Looser terminations so the policy can see longer jump segments early."""
...
class WarmupPPORunnerCfg(BasePPORunnerCfg):
    experiment_name = "unitree_g1_29dof_mimic_jump"
    run_name = "warmup"
