...
@configclass
class CommandsCfg:
    # Retargeted SEED clip "dance_basic_padeburee_270_R_loop_001__A321", wrapped in
    # 1 s quintic default-pose transitions and a 2 s terminal standing hold.
    motion = BaseCommandsCfg().motion.replace(
        motion_file=f"{os.path.dirname(__file__)}/dance_padeburee_270_stand_transition.npz",
        debug_vis=False,
    )
