...
@configclass
class CommandsCfg:
    # Retargeted SEED clip "victory_dance_macarena_change_direction_R_001__A321", wrapped in
    # 1 s quintic default-pose transitions and a 2 s terminal standing hold.
    motion = BaseCommandsCfg().motion.replace(
        motion_file=f"{os.path.dirname(__file__)}/dance_macarena_turn_stand_transition.npz",
        debug_vis=False,
    )
