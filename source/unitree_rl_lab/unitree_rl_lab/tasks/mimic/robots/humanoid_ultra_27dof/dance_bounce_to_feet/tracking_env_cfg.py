...
@configclass
class CommandsCfg:
    # Retargeted SEED clip "dance_hiphop_bounce_to_feet_R_loop_002__A324", wrapped in
    # 1 s quintic default-pose transitions and a 2 s terminal standing hold.
    motion = BaseCommandsCfg().motion.replace(
        motion_file=f"{os.path.dirname(__file__)}/dance_bounce_to_feet_stand_transition.npz",
        debug_vis=False,
    )
